...
(function (factory) {
    ...
    $.extend($.FE.DEFAULTS, {
        ...
        htmlUntouched: false, //https://www.froala.com/wysiwyg-editor/docs/options#htmlUntouched
        ...
        $.FE.MODULES.clean = function (editor) {
            ...
            function _toHTML (el) {
                ...
                if (el.nodeType == Node.TEXT_NODE) {
                    return el.textContent.replace(/\&/g, '&amp;').replace(/\</g, '&lt;').replace(/\>/g, '&gt;').replace(/\u00A0/g, '');//&nbsp; //done by prabhu
                }
        ...
        function _build (command, info, visible) {
            ...
            if (display_selection) {
                ...
                if(info.plugin == "codeView" || info.plugin == "token"){
                    icon = '<span style="font-size: 13px;font-weight: 700 !important;">' + (editor.language.translate(info.title) || '') + '</span>';
                }else{
            ...
            var customstyle="";
            if(info.plugin == "codeView"){
                customstyle= "style=\"width: 70px;\"";
            }else if(info.plugin == "token"){
                customstyle= "style=\"width: 100px;\"";
            }
            var btn = '<button '+customstyle+' id="' + button_id + '"type="button" tabIndex="-1" role="button"' + (info.toggle ? ' aria-pressed="false"' : '') + (info.type == 'dropdown' ? ' aria-controls="drop" aria-expanded="false" aria-haspopup="true"' : '') + (info.disabled ? ' aria-disabled="true"' : '') + ' title="' + (editor.language.translate(info.title) || '') + shortcut + '" class="fr-command fr-btn' + (info.type == 'dropdown' ? ' fr-dropdown' : '') + (' fr-btn-' + editor.icon.getTemplate(info.icon)) + (info.displaySelection ? ' fr-selection' : '') + (info.back ? ' fr-back' : '') + (info.disabled ? ' fr-disabled' : '') + (!visible ? ' fr-hidden' : '') + '" data-cmd="' + command + '"' + popup + modal + '>' + icon + '</button>';
